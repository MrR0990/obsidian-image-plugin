...
import { ImagePluginSettings, CachedImage, CacheMetadata, CacheStats } from './types';
...
export class CacheManager {
	 ...
	 * Add image to cache
	 */
	async add(url: string, data: ArrayBuffer, githubUrl?: string): Promise<CachedImage> {
	...
	private getCleanupCandidates(): CachedImage[] {
		const now = Date.now();
		const protectionMs = this.settings.cacheProtectionDays * 24 * 60 * 60 * 1000;
	 ...
	 */
	private sortByStrategy(candidates: CachedImage[], strategy: string): CachedImage[] {
	...
	private calculateSmartScore(cached: CachedImage): number {
		const now = Date.now();
		const daysSinceAccess = (now - cached.lastAccessedAt) / (24 * 60 * 60 * 1000);
	...
	getStats(): CacheStats {
		...
		const stats: CacheStats = {
			...
			oldestImage: images.length > 0 ? Math.min(...images.map(i => i.createdAt)) : 0,
			newestImage: images.length > 0 ? Math.max(...images.map(i => i.createdAt)) : 0,
			averageSize: this.cacheMetadata.imageCount > 0
